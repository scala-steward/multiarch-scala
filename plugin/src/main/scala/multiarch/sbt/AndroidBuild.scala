...
object AndroidBuild {
  ...
  val androidSdkCacheDir = settingKey[File](
    "Directory where the Android SDK is downloaded/cached if no environment variable is set"
  )
  ...
  val taskSettings: Seq[Setting[_]] = Seq(
    ...
    androidDex := {
      ...
      // Some logging backends (e.g. scribe) use lambdas that D8 mishandles for Scala 3
      // bytecode (VerifyError: wide register index out of range). Exclude them from the
      // fat JAR — consumers can route their Log facade to android.util.Log via reflection.
    ...
    androidPackage := {
      ...
      // Stage app assets from all resource directories into a single assets/ dir.
